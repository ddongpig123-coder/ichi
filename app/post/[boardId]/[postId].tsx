...
export default function PostDetailScreen() {
  ...
  //   （ラウンジ app/lounge/[loungeId]/[postId].tsx にも同じ暫定実装あり — 一緒に消すこと）
  //
  ...
  const loadComments = useCallback(async () => {
    ...
    const c = await fetchComments(schoolDomain, boardId as BoardId, postId);
    setComments(c);
    if (user && c.length) {
      fetchLikedCommentIds(schoolDomain, boardId as BoardId, postId, c.map((x) => x.id), user.uid)
        .then(setLikedComments)
        .catch(() => {});
    }
    return c;
  }, [schoolDomain, boardId, postId, user]);
  ...
  async function handleCommentLike(commentId: string) {
    if (!user || !schoolDomain || likingComment) return;
    setLikingComment(commentId);
    try {
      const res = await toggleCommentLike(schoolDomain, boardId as BoardId, postId, commentId, user.uid);
  ...
  async function submitComment() {
    ...
    try {
      await createComment(
        schoolDomain, boardId as BoardId, postId, user.uid, commentText.trim(),
  ...
  const renderCommentCard = (c: Comment, isReply: boolean) => {
    if (c.deleted) {
      return (
